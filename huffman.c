...
void unpack_block(FILE * fi, FBlock * T, int select)
{
	...
	symbol = get_symbol(fi, HUFF_ID(DC_CLASS, comp[select].DC_HT));//get the category, which corespond to the bits needed
	value = reformat(get_bits(fi, symbol), symbol);//get the real data follow the category
